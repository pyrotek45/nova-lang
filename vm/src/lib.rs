...
impl Vm {
    ...
    pub fn run(&mut self) -> Result<(), NovaError> {
        loop {
            //println!("{}", &self.state.program[self.state.current_instruction]);
            match self.state.next_instruction() {
                ...
                Code::EQUALS => {
                    ...
                    fn compare_heap(state: &State, v1: &Heap, v2: &Heap) -> bool {
                        match (v1, v2) {
                            (Heap::List(l1), Heap::List(l2)) => {
                                if l1.len() != l2.len() {
                                    return false;
                                }
                                for (i, j) in l1.iter().zip(l2.iter()) {
                                    if !compare_heap(state, &state.heap[i], &state.heap[j]) {
                                        return false;
                                    }
                                }
                                true
                            }
                            (Heap::ListAddress(i1), Heap::ListAddress(i2)) => {
                                compare_heap(state, &state.heap[i1], &state.heap[i2])
                            }
                            (a, b) => a == b,
                        }
                    }
                    ...
                    match (v1, v2) {
                        (VmData::List(i1), VmData::List(i2)) => {
                            let result = compare_heap(
                                &self.state,
                                &self.state.heap[&i1],
                                &self.state.heap[&i2],
                            );
                ...
                Code::ASSIGN => {
                    ...
                    match (value, destination) {
                        (item, VmData::StackAddress(index)) => {
                            match (self.state.stack[self.state.offset + index], value) {
                                (VmData::List(d), VmData::Closure(v)) => {
                                    self.state.heap.insert(d, self.state.heap[&v].clone());
                                }
                        ...
                        (item, VmData::List(index)) => {
                            ...
                            match item {
                                VmData::Function(v) => {
                                    self.state.heap.insert(index, Heap::Function(v))
                                }
                                VmData::Int(v) => self.state.heap.insert(index, Heap::Int(v)),
                                VmData::Float(v) => self.state.heap.insert(index, Heap::Float(v)),
                                VmData::Bool(v) => self.state.heap.insert(index, Heap::Bool(v)),
                                VmData::List(v) => {
                                    //dbg!(&self.state.heap[v]);
                                    self.state.heap.insert(index, Heap::ListAddress(v))
                                }
                                VmData::None => self.state.heap.insert(index, Heap::None),
                                VmData::String(v) => {
                                    self.state.heap.insert(index, Heap::StringAddress(v))
                                }
                                VmData::Closure(v) => {
                                    self.state.heap.insert(index, Heap::ClosureAddress(v))
                                }
                                VmData::StackAddress(v) => {
                                    self.state.heap.insert(index, Heap::ListAddress(v))
                                }
                                VmData::Struct(v) => {
                                    self.state.heap.insert(index, Heap::StructAddress(v))
                                }
                                VmData::Char(v) => self.state.heap.insert(index, Heap::Char(v)),
                            };
                        }
                        (a, b) => {
